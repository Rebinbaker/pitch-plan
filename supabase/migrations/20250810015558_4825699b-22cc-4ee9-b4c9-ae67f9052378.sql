-- First, check if trigger exists and remove it
DROP TRIGGER IF EXISTS send_welcome_after_confirmation_trigger ON auth.users;
DROP FUNCTION IF EXISTS public.send_welcome_after_confirmation();

-- Create the trigger function to send welcome email after signup (not confirmation)
