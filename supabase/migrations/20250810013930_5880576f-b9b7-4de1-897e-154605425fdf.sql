-- Drop all existing triggers and functions with CASCADE to clean up properly
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users CASCADE;
DROP TRIGGER IF EXISTS send_welcome_email_trigger ON auth.users CASCADE;
DROP TRIGGER IF EXISTS handle_auth_webhook_trigger ON auth.users CASCADE;
DROP TRIGGER IF EXISTS send_custom_welcome_email_trigger ON auth.users CASCADE;
DROP TRIGGER IF EXISTS on_email_confirmed ON auth.users CASCADE;

-- Drop the functions with CASCADE
DROP FUNCTION IF EXISTS public.handle_auth_webhook() CASCADE;
DROP FUNCTION IF EXISTS public.send_custom_welcome_email() CASCADE;
