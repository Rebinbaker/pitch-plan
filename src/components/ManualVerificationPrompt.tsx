...
import { Camera, Loader2, CheckCircle, ShieldCheck } from 'lucide-react';
...
export const ManualVerificationPrompt = ({ pending, deviceId, onComplete }: Props) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke('submit-manual-verification', {
        body: {
      ...
      <DialogContent className="max-w-md" onPointerDownOutside={(e) => e.preventDefault()}>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Snabb verifiering krävs
          </DialogTitle>
          <DialogDescription>
            Snabb verifiering krävs för aktivt arbetspass. Ta en selfie inom {min}:{String(sec).padStart(2, '0')}.
          </DialogDescription>
        </DialogHeader>
